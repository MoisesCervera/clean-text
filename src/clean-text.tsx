    ...
    const CopyToClipboard = (props: {
        ...
        return (
            <Action
                ...
                icon={Icon.Clipboard}
                onAction={() => {
                    setRecent([props.modification, ...recent.filter((c) => c !== props.modification)].slice(0, 4 + pinned.length));
        ...
        <List isShowingDetail={true} isLoading={!cacheLoaded} selectedItemId={recent[0]}>
            ...
            <List.Section title="All Modifications">
                {Object.keys(functions)
                    .filter(
                        (key) =>
                            !recent.includes(key as ModificationType) &&
                            !pinned.includes(key as ModificationType),
                    )
                    .map((key) => {
                        const modified = modifyTextWrapper(content, key);
                        return <ModificationItem key={key} modification={key as ModificationType} modified={modified.rawText} detail={modified.markdown} />;
                    })}
            </List.Section>
        </List>
    );
